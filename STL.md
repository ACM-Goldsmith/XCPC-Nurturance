...
- 双端队列，支持在队头、队尾插入元素和随机访问

### 8.1 初始化

```cpp
#include <deque>
deque<int> deq; //初始化一个空双端队列
```
...
empty()							//判断队列是否为空
clear()							//清空整个队列
front()/back()					//取队列的开头或者末尾元素
push_back()/pop_back()			//在队列开头或结尾删除元素
push_front()/pop_front()		//在队列开头或结尾增加元素
begin()/end()					//取队列的首迭代器和尾迭代器
[]								//随机访问位于队列某个位置的元素(下标从0开始) e.g. int x = deq[3]
```
